...
const char* g_bridge_path = "WindowsAccessBridge-64.dll";
#ifdef __cplusplus
extern "C" {
#endif
...
#define LOAD_FP(result, type, name) \
    ...
    if ((theAccessBridge.result = \
        ...
        printf("LOAD_FP failed: %s", name); \
        return FALSE; \
    }

	
	extern const char * GetBridgeModulePath();
    BOOL initializeAccessBridge() {

	
		char * dllPath = GetBridgeModulePath();
		if (dllPath == NULL) {
			return FALSE;
		}
		theAccessBridgeInstance = LoadLibraryA(dllPath);
        if (theAccessBridgeInstance != 0) {
			...
			printf("load WindowsAccessBridge-64.dll failed.\n");
            return FALSE;
        }
    }
    ...
    BOOL shutdownAccessBridge() {
        ...
        if (theAccessBridgeInstance != (HANDLE) 0) {
           FreeLibrary(theAccessBridgeInstance);
            return TRUE;
        }
        return FALSE;
    }
