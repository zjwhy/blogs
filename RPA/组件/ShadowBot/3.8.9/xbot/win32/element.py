...
class Win32Element(metaclass=abc.ABCMeta):
    ...
    def input(self, text: str, simulative=True, append=False, contains_hotkey=False, send_key_delay=50, focus_timeout=1000, delay_after=1, anchor=None) -> typing.NoReturn:
        ...
        * @param anchor, 锚点, 鼠标点击元素的位置以及偏移量元组, 可为 `None`, 默认值为 `None` 为 `None` 时默认点击目标中心且无偏移量, 参数结构如下:
        ...
        sudoku_part, offset_x, offset_y = (
            'middleCenter', 0, 0) if anchor is None else anchor
        self._invoke('Input', {'text': text, 'simulative': simulative,
                               'append': append, 'containsHotkey': contains_hotkey,
                               'sudokuPart': sudoku_part,
                               'offsetX': offset_x,
                               'offsetY': offset_y,
    ...
    def get_bounding(self, to96dpi=True) -> tuple:
        '''
        获取win32元素边框信息
        * @param to96dpi, 是否将获取的到元素边框信息转换为96dpi下的值, 默认值为`True`需要转换
        * @return `tuple`, 返回一组win32元素边框信息, 如('x', 'y', 'width', 'height')
        '''
        bounding = self._invoke('GetBounding', {'to96dpi': to96dpi})
    ...
    def _create_element(self, eid) -> Win32Element:
        if eid.startswith('SAP'):
            return SAPElement('Win32Element', eid)
        else:
            return Win32Element('Win32Element', eid)
