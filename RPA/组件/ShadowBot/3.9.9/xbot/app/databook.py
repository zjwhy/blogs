...
def set_row(row_num, values, begin_column_name='A') -> typing.NoReturn:
    ...
    * @param begin_column_name, 设置开始的单元格列名或列号, 默认值为`'A'`
    '''
...
def get_range(area_begin_row_num, area_begin_column_name, area_end_row_num, area_end_column_name) -> typing.List[typing.List[str]]:
    ...
    * @param area_begin_column_name, 起始单元格列名或列号, 列名从'A'或1开始
    * @param area_end_row_num, 结束单元格行号, 行号从1开始
    * @param area_end_column_name, 结束单元格列名或列号, 列名从'A'或1开始
...
def set_range(row_num, col_name, values) -> typing.NoReturn:
    ...
    * @param col_name, 设置区域起始列名或列号, 列名从'A'或1开始
...
def import_data(file_path, sheet_name):
    '''
    数据表格导入
    * @param file_path, 填写要导入的文件路径
    * @param sheet_name, Sheet页名称
    '''
    if not os.path.isfile(file_path):
        raise ValueError(f'文件{file_path}不存在, 请检查后重试！')

    _execute(f'Workbook.ImportData', { 'filePath': file_path, 'sheetName': sheet_name })


def export_data(file_path):
    '''
    数据表格导出
     * @param file_path, 设置导出文件的保存路径
    '''
    _execute(f'Workbook.ExportData', { 'filePath': file_path })