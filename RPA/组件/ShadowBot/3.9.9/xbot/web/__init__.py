...
def get_all(mode='cef', *, title=None, url=None, use_wildcard=False) -> List[WebBrowser]:
    ...
    * @return `List[WebBrowser]`, 返回已打开的网页对象列表
    ...
    automation = _create_web_automation(mode)
    browsers = automation.get_all(
        title=title, url=url, use_wildcard=use_wildcard)
    return browsers
...
def close_all(mode='cef', *, task_kill=False) -> NoReturn:
    """
    关闭所有网页
    * @param mode, 浏览器类型
        * `'cef'`, 影刀浏览器,
        * `'chrome'`, Google Chrome浏览器
        *`'ie'` Internet Explorer浏览器
    * @param task_kill, 是否终止浏览器进程, 默认False. 若为True, 将强制结束属于当前用户的指定浏览器进程和由它启动的所有子进程
    """

    automation = _create_web_automation(mode)
    automation.close_all(task_kill=task_kill)


def handle_save_dialog(file_folder, dialog_result='ok', mode='cef', *, file_name=None, simulative=False, clipboard_input=True, wait_appear_timeout=20, force_ime_ENG=False, send_key_delay=50, focus_timeout=1000) -> NoReturn:
    """
    处理网页下载对话框
...
def handle_upload_dialog(filenames, dialog_result='ok', mode='cef', *, simulative=False, clipboard_input=True, wait_appear_timeout=20, force_ime_ENG=False, send_key_delay=50, focus_timeout=1000) -> NoReturn:
    """
    处理网页上传对话框
    ...
    * @param focus_timeout, 焦点超时时间(获取焦点和输入操作的间隔), 默认为1000ms
    ...
    if not isinstance(filenames, list):  # 兼容单个路径的情况
        filenames = [filenames]
    automation = _create_web_automation(mode)
    automation.handle_upload_dialog(dialog_result,
                                    filenames,
                                    simulative=simulative,
                                    clipboard_input=clipboard_input,
                                    wait_appear_timeout=wait_appear_timeout,
