...
class WebAutomation(metaclass=ABCMeta):
    ...
    def get_all(self, *, title=None, url=None, use_wildcard=False) -> List[WebBrowser]:
        ...
        * @return `List[WebBrowser]`, 返回已打开的网页对象列表
        ...
        bids = self._invoke('GetAllBrowsers', {
                            'title': title, 'url': url, 'useWildcard': use_wildcard})
    ...
    def get_cookies(self, url, *, name=None, domain=None, path=None) -> list:
        ...
        * @return `list[dict]`, 返回筛选到的cookie列表, 列表项键包括'domain'、'expirationDate'、'name'、'value'、'httpOnly'..., 集合中的值可以通过比如 item['value'] 的方式获取"
        ...
        cookies = self._invoke(
            'GetCookies', {'url': url, 'name': name, 'domain': domain, 'path': path})
        return cookies

    def close_all(self, *, task_kill=False) -> NoReturn:
        """
        关闭全部已打开的网页
        """

        self._invoke('CloseAllBrowsers', {'taskKill': task_kill})

    def handle_save_dialog(self, dialog_result, file_folder, *, file_name=None, simulative=False, clipboard_input=True, wait_appear_timeout=20, force_ime_ENG=False, send_key_delay=50, focus_timeout=1000) -> str:
        """
        处理网页下载对话框
    ...
    def handle_upload_dialog(self, dialog_result, filenames, *, simulative=False, clipboard_input=True, wait_appear_timeout=20, force_ime_ENG=False, send_key_delay=50, focus_timeout=1000) -> NoReturn:
        """
        处理网页上传对话框
