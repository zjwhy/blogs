...
def get_text_length(**args) -> int:
    ...
    valid_multi('文本内容', text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
...
def append_text_in_newline(**args) -> str:
    ...
    valid_multi('原始文本', text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    valid_multi('追加文本', new_line_text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
...
def sub_text(**args) -> str:
    ...
    if args['sub_way'] == 'length':
        ...
        if sub_length < 0:
            raise ValueError(f'截取长度不能为{sub_length}')
...
def pad_text(**args) -> str:
    ...
    valid_multi('原始文本', text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    valid_multi('用于补齐的文字', pad_text, (ValidPattern.NotNone, (ValidPattern.Type, str)))

    if total_length < 0:
        raise ValueError(f'总长度不能为{total_length}')
...
def join_list_to_text(**args) -> str:
    ...
    if delimiter_way == 'no':
        ...
        valid_multi('自定义的连接符', join_chars, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    else:
        standard_delimiter = args['standard_delimiter']
        num_standard_delimiter = parseint_from_args(args, 'num_standard_delimiter')
        if num_standard_delimiter < 0:
            raise ValueError(f'连接符的数量不能为{num_standard_delimiter}')
...
def split_text_to_list(**args) -> typing.List[str]:
    ...
    valid_multi('待转换的文本', text_to_split, (ValidPattern.NotNone, (ValidPattern.Type, str)))

    # 2、分割
    text_list = []
    if delimiter_way == 'custom':
        valid_multi('自定义的分割符', custom_delimiter, (ValidPattern.NotNone, (ValidPattern.Type, str)))
        if custom_delimiter == '':
            raise ValueError('自定义的分割符不能为空')
...
def extract_content_from_text(**args) -> typing.Any:
    ...
    valid_multi('内容源', text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    
    valid_multi('正则表达式', regular_pattern, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    if regular_pattern == '':
...
def replace_content_from_text(**args)->str:
    ...
    valid_multi('内容源', text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
    valid_multi('替换为', dest_text, (ValidPattern.NotNone, (ValidPattern.Type, str)))

    if replace_way == "content":
        valid_multi('被替换内容', repalce_text, (ValidPattern.NotNone, (ValidPattern.Type, str)))
        if repalce_text == '':
            raise ValueError('被替换内容不能为空')
        if just_get_first:
