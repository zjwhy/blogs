...
def loop_data_from_workbook_with_return_item_location(**args) -> typing.Any:
    ...
    if loop_way == 'loop_row':
        ...
        begin_column_name = _convert_to_column_name(args['begin_column_name'])
...
def import_data(**args):
    ...
    databook.import_data(file_path, sheet_name)
...
def export_data(**args):
    ...
    if folder_source == 'desktop':
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, sub_key) as key:
            save_folder = winreg.QueryValueEx(key, 'Desktop')[0]
    elif folder_source == 'download':
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, sub_key) as key:
            save_folder = winreg.QueryValueEx(key, '{374DE290-123F-4565-9164-39C4925E467B}')[0]
    elif folder_source == 'tmp':
        save_folder = os.environ.get('tmp')
    elif folder_source == 'custom':
        save_folder = args['custom_folder_path']
    
    save_path = f'{save_folder}\\{file_name}'
    databook.export_data(save_path)
    return save_path
...
def _loop_data(args) -> typing.Any:
    ...
    begin_column_name = _convert_to_column_name(args['begin_column_name'])
    end_column_name = _convert_to_column_name(args['end_column_name'])

    range_begin_row_num = args['range_begin_row_num']
    range_begin_column_name = _convert_to_column_name(args['range_begin_column_name'])
    range_end_row_num = args['range_end_row_num']
    range_end_column_name = _convert_to_column_name(args['range_end_column_name'])
...
def _convert_to_column_name(column) -> str:
    '''
    获取列名，兼容处理，支持数据表格的列号读写
    * @params column ：
    * 1、列号
    * 2、列名称
    '''
    # 1、如果是列号，就转成列名
    if column.isdigit():
        return excel_column_index_to_name(int(column))
    # 2、如果是列名，就和以前的处理逻辑保持一致
    else:
        return column