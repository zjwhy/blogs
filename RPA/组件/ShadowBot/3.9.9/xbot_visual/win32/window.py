...
def _get_window_by_type(args):
    ...
    if win_type == 'window_instance':
        ...
        selector_or_element = args['selector']
        if isinstance(selector_or_element, Selector):
            return win32.get_by_selector(selector_or_element, timeout=timeout)
        elif isinstance(selector_or_element, Win32Element):
            return win32.get_by_element(selector_or_element)
        else:
            raise ValueError(f'窗口类型错误{win_type} : {window}')
    ...
    elif win_type == "active":
        return win32.get_active()
