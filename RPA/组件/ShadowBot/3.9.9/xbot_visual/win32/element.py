...
def get_element(**args):
    ...
    if isinstance(selector, Selector):
        window = _get_window(args, attribute_name='selector')
        return window.find(selector, timeout=timeout)
    else:
        return selector
...
def get_all_elements(**args):
    ...
    if isinstance(selector, Selector):
        window = _get_window(args, attribute_name='selector')
        return window.find_all(selector, timeout=timeout)
    else:
...
def iter_all_elements(**args):
    ...
    timeout = parseint_from_args(args, 'timeout')
    window = _get_window(args, attribute_name='selector')
    return window.find_all(args['selector'], timeout=timeout)
...
def wait(**args):
    ...
    # 2、等待出现/消失
    window = _get_window(args, attribute_name='element')
    if state == 'appear':
        return window.wait_appear(element, timeout)
    else:
        return window.wait_disappear(element, timeout)
...
def _element(args, attribute_name='element') -> Win32Element:
    ...
    if element is None:
        ...
        window = _get_window(args, attribute_name=attribute_name)
        return window.find(element)  # 如果是选择器，需要先在窗口中找一下
    else:
        return element  # 如果是Win32Element对象就直接返回


def _get_window(args, attribute_name='element') -> Win32Window:
    window = args['window']
    if isinstance(window, str):
        element = args.get(attribute_name, None)
        if isinstance(element, Selector):
            timeout = parseint_from_args(args, 'timeout', default=5)
            return win32.get_by_selector(element, timeout=timeout)
        else:
            return win32.get_by_element(element)
    else:
        return window
