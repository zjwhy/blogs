...
def connect(**args):
    ...
    if connect_source == 'current':
        ...
        if os.environ.get('ANDROID_HOME') is None:
            raise ValueError('请先配置系统环境变量ANDROID_HOME的值为Android SDK的安装目录')
        ...
        if android_version == '':
            raise ValueError('无法获取手机系统的版本号，请确认手机是否连接')
        return mobile.connect(f'http://127.0.0.1:{port}/wd/hub', 'Android', android_version, 'DeviceName', additional_capabilities = {'newCommandTimeout': 0})
    else:
